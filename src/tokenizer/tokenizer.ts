...
import { LogCategory, logCatMessage, LogLevel } from "../logger";
...
export class Tokenizer {
    ...
    private static async runTokenizer(document: TextDocument) {
        logCatMessage(LogLevel.Info, LogCategory.Tokenizer, `Running tokenizer on document: ${document.uri}`);
    ...
    private executeRangePattern(bestMatch: ScanResult, source: string, parentNode: TreeNode) {
        ...
        if (!coverageResult.valid) {
            ...
            for (const gap of coverageResult.gaps) {
                ...
                const text = this.document.getText({ start: gapStartPos, end: gapEndPos });
